...
def build_prompt(query_type: str, query_text: str, code_context: CodeContext) -> str:
    ...
    prompt = f"""
As a senior software engineer, analyze this {language} code:
...
Please provide helpful, actionable insights.
"""
    return prompt
...
async def query_code(request: QueryRequest):
    ...
    try:
        if gemini_client:
            ...
            ai_response = f"""
This {request.code_context.language} code in {request.code_context.file_path} shows:

Analysis for: {request.query_text}
Task: {request.query_type.value}

The code appears to be well-structured. Here are some observations:
1. The function follows standard conventions
2. Consider adding error handling
3. Documentation could be improved

This is a mock response - configure GOOGLE_API_KEY for real AI analysis.
"""
            suggestions = [
                CodeSuggestion(
        ...
        return QueryResponse(
            ...
            code_examples=["# Example code will be generated"],
